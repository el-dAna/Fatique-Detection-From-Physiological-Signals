...
@dataclass
class PREPROCESSING_VARS:
    # directory of dataset. structure is
    """
    Base_dir
     -subject1
      ...
      -Subject2
        ...
        -Subject2SpO2HR.csv
    """
    BASE_DIR = "/workspaces/Fatique-Detection-From-Physiological-Signals/temp/Fatigue_Detection-main/HealthySubjectsBiosignalsDataSet"
...
if __name__ == "__main__":
    ...
    print(
        f"Subjects {PREPROCESSING_VARS.SUBJECTS} used for Training. A total of {len(os.listdir(PREPROCESSING_VARS.BASE_DIR)) - PREPROCESSING_VARS.TOTAL_SUBJECT_NUM} reserved for validation."
    )

    # calling SortSPO2HR and SortAccTempEDA functions from preprocessing functions
    SPO2HR, SPO2HR_attributes_dict = SortSPO2HR(
        PREPROCESSING_VARS.BASE_DIR, PREPROCESSING_VARS.SUBJECTS
    )
    AccTempEDA, AccTempEDA_attributes_dict = SortAccTempEDA(
        PREPROCESSING_VARS.BASE_DIR, PREPROCESSING_VARS.SUBJECTS
    )
    ...
    (
        ...
        phy_emo_cog_indices,
        all_attributes,
    ) = necessary_variables("takes_nothing__HAHAHA")

    SPO2HR_resized, AccTempEDA_resized = resize_to_uniform_lengths(
        PREPROCESSING_VARS.TOTAL_SUBJECT_NUM,
        categories,
        attributes_dict,
        SPO2HR_target_size,
        SPO2HR,
    ...
    AccTempEDA_DownSampled = sanity_check_2_and_DownSamplingAccTempEDA(
        PREPROCESSING_VARS.TOTAL_SUBJECT_NUM,
        categories,
        attributes_dict,
        SPO2HR_target_size,
    ...
    if os.path.exists(path_to_saved_vars) and overide_previously_saved_file == True:
        ...
        os.path.exists(path_to_saved_vars) == True
        and overide_previously_saved_file == False
    ):
        print("Hey")
    else:
        print("Hey 2")
